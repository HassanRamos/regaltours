...
function call_get_api(product_type,search_term,key){
	 $.ajax({
                type: "GET",
                url: "https://regal-tours.com/v1/"+product_type+"/search?destination=" + search_term,
                dataType: "json",
                success: function (result, status, xhr) {
					var selectElement = document.getElementById(key);
					var str = "";
					for (var i = 0; i <= result["data"].length-1; i++) {
					  str += '<option value="'+result["data"][i]["destination"]+'" />'; }
...
function call_post_api(product_type,payload){
	 $.ajax({
                type: "POST",
                url: "https://regal-tours.com/v1/"+product_type+"/search",
...
function search_hotels_offer(){
	console.log("+++++++++")
	var payload = {data: { no_of_children : document.getElementById('hotel_rooms_no').value,
						   no_of_adults: document.getElementById('hotel_adults_no').value,
			               no_of_rooms : document.getElementById('hotel_rooms_no').value,
						   destination: document.getElementById('hotel_destination').value,
						   checkout_date: document.getElementById('hotel_checkout_date').value,
				           checkin_date: document.getElementById('hotel_checkin_date').value}}

	call_post_api("hotel",payload)
...
if (window.location.pathname === "/home"){
	var hotel_destination = document.getElementsByName("hotel_destination");
	hotel_destination[0].addEventListener('input',function (e){
		call_get_api("hotel",this.value,"hotel_destinations");
	})
}
...
if (window.location.pathname === "/hotels") {
	document.addEventListener('DOMContentLoaded', function () {
		var hotel_data = $('#list_of_hotels').data("hotels")["batch"];
		var hotel_html = "";
		for (var i = 0; i < hotel_data.length; i++) {
			for (var j = 0; j < 2; j++) {

				hotel_html += '<div class="col-md-6">' +
					'<div class="recom-item border">' +
					'<div class="recom-media">' +
					'<a href="hotels-details.html">' +
					'<div class="pic">' +
					'<img id="hotel_images' + i.toString() + j.toString() + '" name="hotel_images" src="' + hotel_data[i][j]["HotelInfo"]["HotelPicture"] + '" data-at2x="' + hotel_data[i][j]["HotelInfo"]["HotelPicture"] + '" alt>' +
					'</div>' +
					'</a>' +
					'<div class="location">' +
					'<i class="flaticon-suntour-map"></i> Vienna, Austria' +
					'</div>  ' +
					'</div>' +
					'<div class="recom-item-body">' +
					'<a href="hotels-details.html">' +
					'<h6 class="blog-title"> ' + hotel_data[i][j]["HotelInfo"]["HotelName"] + '</h6>' +
					'</a>' +
					'<div class="stars stars-5"></div>' +
					'<div class="recom-price">' +
					'<span class="font-4">$ ' + hotel_data[i][j]["MinHotelPrice"]["TotalPrice"] + '</span>' +
					'</div>' +
					'</div>' +
					'</div> </div></div>'
			}
		}
		document.getElementById("list_of_hotels").innerHTML = hotel_html

		var images = document.getElementsByName("hotel_images");
		for (var k = 0; k < images.length; k++) {
			...
			img.onerror = function () {
				document.getElementById(img.id).src = "../static/pic/recomended/4.jpg";
			}
			img.id = images[k].id;
